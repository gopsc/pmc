...
namespace qing {
	...
	FrameBuf::FrameBuf(const char *path, int rotate, int fontsize)
	  : Drawable(0, 0, 0, 0, rotate, fontsize) {
		...
		double delta_bpp = (double)delta / var_info.bits_per_pixel;


		std::cout << "virtual size:\t" << v_size << std::endl;
		std::cout << "logic   size:\t" << logic_size<< std::endl;
		std::cout << "delta:       \t" << delta<< std::endl;
		std::cout << "delta   x:   \t" << delta_x<< std::endl;
		std::cout << "delta   y:   \t" << delta_y<< std::endl;
		std::cout << "delta   bpp: \t" << delta_bpp<< std::endl;
		if (delta == 0) {
